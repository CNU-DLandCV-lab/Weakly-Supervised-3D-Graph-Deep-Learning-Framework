...
def spec_hier_cluster_pool(inputs , pool_method = 'max' , csize = 4, use_dist_adj = False, fast_approx = False, include_eig = False ):
    ...
    while(K > csize and K % csize == 0):
        ...
        if (not fast_approx) or (K == in_shape[2]):
            ...
            eig_2nd_sorted , sort_ind = tf.nn.top_k(input = partition_vect , k=int(K), sorted=True)
            # B*N , K
            sort_ind = tf.reshape(sort_ind , [in_shape[0] * in_shape[1] , int(K) ])
            # inputs: B N K m -> BN , K,m
           # print(inputs)
            inputs = tf.reshape(inputs , [in_shape[0] * in_shape[1] ,int(K) , in_shape[3]])
    ...
    #    print(inputs)
        ...
        inputs = tf.reshape(tf.transpose(inputs, perm = [0,2,1]) ,
                            (in_shape[0] , in_shape[1] , int(K) , in_shape[3]))
